...
func TestKalibrrAdapter_Fetch_NormalizesSalaryAmount(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			expectedSalaryRange: "15000000 - 25000000",
		},
		{
			...
			expectedSalaryRange: "15000000 - 61754809",
		},
	}
	...
	for _, testCase := range tests {
		...
		t.Run(testCase.name, func(t *testing.T) {
			...
			if result.Jobs[0].SalaryRange != testCase.expectedSalaryRange {
				t.Fatalf("expected salary_range %q, got %q", testCase.expectedSalaryRange, result.Jobs[0].SalaryRange)
			}
		})
	}
}
...
func TestKalibrrAdapter_Fetch_AllowsEmptySalaryStrings(t *testing.T) {
	...
	if result.Jobs[0].SalaryRange != "" {
		t.Fatalf("expected salary_range empty, got %q", result.Jobs[0].SalaryRange)
	}
}

func TestKalibrrAdapter_Fetch_SupportsMaximumOnlySalary(t *testing.T) {
	t.Parallel()
	...
	if result.Jobs[0].SalaryMax == nil || *result.Jobs[0].SalaryMax != 12000000 {
		t.Fatalf("expected salary_max %d, got %+v", 12000000, result.Jobs[0].SalaryMax)
	}
	if result.Jobs[0].SalaryRange != "<= 12000000" {
		t.Fatalf("expected salary_range %q, got %q", "<= 12000000", result.Jobs[0].SalaryRange)
	}
}
