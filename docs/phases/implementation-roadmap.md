...
- **Phase 4 backend increment 1** sudah mulai diimplementasikan (coupon-enabled checkout: validasi `coupon_code` ke Mayar + invoice amount diskon + metadata amount response), diikuti hardening `salary normalization` untuk mapping scraper (`min-only`/`max-only`/`exact`/`range`) termasuk comparator (`<= ...`) dan shorthand bulanan (`Rp 8 – Rp 12 per month`).
...
| M5 | Eksekusi Phase 4 backend | 🟡 In Progress | Increment 1 aktif: coupon-enabled checkout pada billing + hardening salary normalization mapping scraper (comparator + shorthand monthly range) |
