...
Catatan progress M5 saat ini:

- bug `ON CONFLICT` partial index pada `billing_repository.go` sudah diperbaiki: clause diupdate ke `ON CONFLICT (provider, mayar_transaction_id) WHERE mayar_transaction_id IS NOT NULL DO NOTHING` agar cocok dengan partial unique index di PostgreSQL,
- `.env` backend dibersihkan dari variabel Mayar yang stale (`MAYAR_BASE_URL`, `MAYAR_API_KEY`, `MAYAR_REQUEST_TIMEOUT`, `MAYAR_MAX_RETRIES`, `BILLING_WEBHOOK_TOKEN` versi Mayar),
- `go test ./...` dan `go build ./...` pass setelah fix.
...
| **[Increment 2]** `GET /jobs/titles` — autocomplete judul lowongan | 🟡 | `apps/api/internal/adapter/http/handler/jobs_handler.go` (`SearchTitles`), `apps/api/internal/app/jobs/service.go`, `apps/api/internal/domain/job/job.go` (`TitleSearchQuery`, `SearchTitles()`), `apps/api/internal/adapter/persistence/postgres/jobs_repository.go`, `apps/api/internal/adapter/persistence/memory/jobs_repository.go`, `apps/api/internal/adapter/http/router/router.go` | `apps/api/internal/adapter/http/handler/jobs_handler_test.go` | local gate: `go vet` clean pada paket terkait | `docs/api/jobs.md`, `docs/frontend/features/ai-career-copilot.md` |
| **[Increment 2]** multi-kata full-text search (`q` split + AND LIKE) | 🟡 | `apps/api/internal/adapter/persistence/postgres/jobs_repository.go` (`Search()`, multi-word split), `apps/api/migrations/000006_jobs_search_index.up.sql`, `apps/api/migrations/000006_jobs_search_index.down.sql` | `apps/api/internal/adapter/http/handler/jobs_handler_test.go` | local gate: `go vet` clean pada paket terkait | `docs/api/jobs.md`, `docs/features/job-search.md` |
...
| Item                                         | Status | Code Evidence | Test Evidence | CI Evidence | Docs/ADR Evidence |
| -------------------------------------------- | ------ | ------------- | ------------- | ----------- | ----------------- |
| advanced billing (coupon/installment/qrcode) | 🟡     | `apps/api/internal/app/billing/service.go`, `apps/api/internal/adapter/http/handler/billing_handler.go`, `apps/api/internal/adapter/billing/mayar/client.go`, `apps/api/internal/platform/config/config.go`, `apps/api/.env.example`, `apps/api/pkg/errcode/codes.go` | `apps/api/internal/app/billing/service_test.go`, `apps/api/internal/adapter/http/handler/billing_handler_test.go`, `apps/api/internal/adapter/billing/mayar/client_test.go`, `apps/api/test/integration/billing_checkout_flow_test.go` | local gate pass: `make -C apps/api lint test build check-migrations` | `docs/api/billing.md`, `docs/api/errors.md`, `docs/architecture/local-development-runbook.md`, `docs/features/optional-features.md`, `docs/phases/implementation-roadmap.md` |
| salary normalization lanjutan                | ✅     | `apps/api/internal/adapter/scraper/source/helpers.go`, `apps/api/internal/adapter/scraper/source/glints_adapter.go`, `apps/api/internal/adapter/scraper/source/kalibrr_adapter.go`, `apps/api/internal/adapter/scraper/source/jobstreet_adapter.go`, `apps/api/internal/adapter/http/handler/jobs_handler.go` | `apps/api/internal/adapter/scraper/source/helpers_test.go`, `apps/api/internal/adapter/scraper/source/glints_adapter_test.go`, `apps/api/internal/adapter/scraper/source/kalibrr_adapter_test.go`, `apps/api/internal/adapter/scraper/source/jobstreet_adapter_test.go`, `apps/api/internal/adapter/http/handler/jobs_handler_test.go`, `go test ./internal/adapter/scraper/source ./internal/adapter/http/handler ./internal/adapter/persistence/memory` | local gate pass: `make -C apps/api lint test build check-migrations` | `docs/api/jobs.md`, `docs/features/job-aggregation.md`, `docs/features/source-scraping-playbook.md`, `docs/phases/implementation-checklist.md` |
...
| AI job-fit summary (premium insight)                                    | 🟡     | `apps/api/internal/app/ai/service.go`, `apps/api/internal/adapter/ai/openai/client.go`, `apps/api/internal/adapter/http/handler/ai_handler.go`, `apps/api/internal/adapter/http/router/router.go` | `apps/api/internal/app/ai/service_test.go`, `apps/api/internal/adapter/ai/openai/client_test.go`, `apps/api/internal/adapter/http/handler/ai_handler_test.go`, `apps/api/internal/adapter/http/router/router_test.go` | local gate pass: `make -C apps/api lint test build check-migrations` | `docs/api/ai.md`, `docs/phases/implementation-roadmap.md`, `docs/features/optional-features.md` |
| AI cover letter draft API (premium capability)                          | 🟡     | `apps/api/internal/app/ai/service.go`, `apps/api/internal/adapter/ai/openai/client.go`, `apps/api/internal/adapter/http/handler/ai_handler.go`, `apps/api/internal/adapter/http/router/router.go` | `apps/api/internal/app/ai/service_test.go`, `apps/api/internal/adapter/ai/openai/client_test.go`, `apps/api/internal/adapter/http/handler/ai_handler_test.go`, `apps/api/internal/adapter/http/router/router_test.go` | local gate pass: `make -C apps/api lint test build check-migrations` | `docs/api/ai.md`, `docs/phases/implementation-roadmap.md`, `docs/features/optional-features.md` |
| AI interview prep API (tier-based capability)                           | 📝     | -             | -             | -           | `docs/phases/implementation-roadmap.md`, `docs/features/optional-features.md` |
