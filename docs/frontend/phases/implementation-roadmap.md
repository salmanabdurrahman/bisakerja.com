...
- **M5 frontend follow-up** sudah mulai diimplementasikan untuk kontrak backend Phase 4 increment 1 (coupon-enabled checkout + safe rendering untuk job description HTML + salary display normalization), termasuk pesan validasi checkout yang lebih actionable untuk `INVALID_REDIRECT_URL`.
...
| M5 | Phase 4 backend follow-up | 🟡 In Progress | Coupon-enabled checkout UX + discovery hardening (sanitized job description rendering + salary fallback normalization) + actionable redirect validation message |
